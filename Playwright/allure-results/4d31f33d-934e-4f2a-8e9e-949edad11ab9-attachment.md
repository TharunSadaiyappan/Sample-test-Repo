...
Call log:
  - waiting for getByRole('button', { name: 'User Management' })

    at C:\Users\CVHS\Documents\VS Code\tests\PracticePlaywright\Vassist.spec.js:42:69
```
...
> 42 |         await page.getByRole('button', { name: 'User Management' }).click();
  ...
  63 |
  64 |         // Uncomment below to save the user
  65 |         // await page.getByRole('button', { name: 'Save Changes' }).click();
  66 |
  67 |         await context.close(); // Cleanup
  68 |     }
  69 | });
  70 |
```